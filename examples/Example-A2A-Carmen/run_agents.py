...
Reads YAML configuration and starts agents accordingly.
"""
import yaml
import subprocess
import sys
...
class AgentRunner:
    ...
    def start_agent(self, agent_config):
        ...
        # e.g., "agents.tools_agent" -> "agents/tools_agent.py"
        module_path = agent_module.replace('.', os.sep) + '.py'
        agent_file = self.script_dir / module_path

        # Start the agent as a subprocess by running the file directly
        cmd = [sys.executable, '-u', str(agent_file)]  # -u for unbuffered output

        # Open log files in binary mode to avoid buffering issues
        stdout_file = open(stdout_log, 'wb', buffering=0)
        stderr_file = open(stderr_log, 'wb', buffering=0)

        # Use start_new_session=True on Unix/Linux to prevent signal propagation
        # This prevents the child process from receiving signals from the parent
        process = subprocess.Popen(
            cmd,
            env=env,
            cwd=self.script_dir,  # Run from the multi_agent_system directory
            stdout=stdout_file,
            stderr=stderr_file,
            start_new_session=True  # Prevent signal propagation from parent
        )
        ...
        # Give it a moment to start (increased for WSL compatibility)
        time.sleep(8)
    ...
    def start_all_agents(self):
        ...
        print(f"\n{'='*60}")
        print(f"Starting Multi-Agent System: {self.config['job']['name']}")
        print(f"{'='*60}\n")
        ...
        if strategy == 'sequential':
            # Start tools agent first (spoke), then general agent (hub)
        ...
        print(f"\n[INFO] Entry Point: General Agent on port 9001")
        print(f"[INFO] Send queries to: http://localhost:9001/query")
    ...
    def verify_agent_connectivity(self):
        ...
        # Wait a bit more for all servers to be ready (increased for WSL)
        ...
        for proc_info in self.processes:
            ...
            # Retry logic - try up to 5 times with increasing wait (increased for WSL)
            max_retries = 5
            retry_delay = 3
            success = False
            ...
            if not success:
                print(f"✗ {name} is not accessible after {max_retries} attempts")
                all_healthy = False
