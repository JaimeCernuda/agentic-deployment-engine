...
class TestWeatherAgent:
    ...
    def test_init_creates_agent_with_default_port(self) -> None:
        ...
        with patch("agents.weather_agent.create_sdk_mcp_server") as mock_create:
            mock_server = MagicMock()
            mock_create.return_value = mock_server

            with patch("agents.weather_agent.BaseA2AAgent.__init__", return_value=None):
    ...
    def test_init_accepts_custom_port(self) -> None:
        ...
        with patch("agents.weather_agent.create_sdk_mcp_server") as mock_create:
            ...
            with patch("agents.weather_agent.BaseA2AAgent.__init__") as mock_init:
    ...
    def test_get_skills_returns_weather_skills(self) -> None:
        ...
        with patch("agents.weather_agent.create_sdk_mcp_server"):
            with patch("agents.weather_agent.BaseA2AAgent.__init__", return_value=None):
                from agents.weather_agent import WeatherAgent
                agent = WeatherAgent()
...
class TestMapsAgent:
    ...
    def test_init_creates_agent_with_default_port(self) -> None:
        ...
        with patch("agents.maps_agent.create_sdk_mcp_server") as mock_create:
            mock_server = MagicMock()
            mock_create.return_value = mock_server

            with patch("agents.maps_agent.BaseA2AAgent.__init__", return_value=None):
    ...
    def test_init_accepts_custom_port(self) -> None:
        """Should accept custom port configuration."""
        with patch("agents.maps_agent.create_sdk_mcp_server"):
            with patch("agents.maps_agent.BaseA2AAgent.__init__") as mock_init:
                mock_init.return_value = None
                from agents.maps_agent import MapsAgent
...
class TestControllerAgent:
    ...
    def test_init_creates_agent_with_default_port(self) -> None:
        ...
        with patch("agents.controller_agent.create_a2a_transport_server") as mock_create:
            mock_server = MagicMock()
            mock_create.return_value = mock_server

            with patch("agents.controller_agent.BaseA2AAgent.__init__") as mock_init:
                mock_init.return_value = None
    ...
    def test_init_with_default_connected_agents(self) -> None:
        ...
        with patch("agents.controller_agent.create_a2a_transport_server"):
            with patch("agents.controller_agent.BaseA2AAgent.__init__") as mock_init:
                mock_init.return_value = None
                from agents.controller_agent import ControllerAgent
                agent = ControllerAgent()
    ...
    def test_get_skills_returns_coordination_skills(self) -> None:
        ...
        with patch("agents.controller_agent.create_a2a_transport_server"):
            with patch("agents.controller_agent.BaseA2AAgent.__init__", return_value=None):
                from agents.controller_agent import ControllerAgent
                agent = ControllerAgent()
                skills = agent._get_skills()
...
class TestWeatherAgentMain:
    ...
    def test_main_reads_port_from_environment(self) -> None:
        ...
        with patch.dict(os.environ, {"AGENT_PORT": "8888"}):
            with patch("agents.weather_agent.WeatherAgent") as mock_agent_class:
                mock_agent = MagicMock()
                mock_agent_class.return_value = mock_agent

                from agents.weather_agent import main
                main()
...
class TestMapsAgentMain:
    ...
    def test_main_reads_port_from_environment(self) -> None:
        ...
        with patch.dict(os.environ, {"AGENT_PORT": "7777"}):
            with patch("agents.maps_agent.MapsAgent") as mock_agent_class:
                mock_agent = MagicMock()
                mock_agent_class.return_value = mock_agent

                from agents.maps_agent import main
                main()
...
class TestControllerAgentMain:
    ...
    def test_main_reads_port_from_environment(self) -> None:
        ...
        with patch.dict(os.environ, {"AGENT_PORT": "6666"}, clear=True):
            with patch("agents.controller_agent.ControllerAgent") as mock_agent_class:
                mock_agent = MagicMock()
                mock_agent_class.return_value = mock_agent

                from agents.controller_agent import main
                main()
                    ...
                    with patch("agents.weather_agent.BaseA2AAgent.__init__", return_value=None):
                        with patch("agents.maps_agent.BaseA2AAgent.__init__", return_value=None):
                            with patch("agents.controller_agent.BaseA2AAgent.__init__", return_value=None):
                                ...
                                agents = [WeatherAgent(), MapsAgent(), ControllerAgent()]
                                required_fields = ["id", "name", "description", "tags", "examples"]

                                for agent in agents:
                                    for skill in agent._get_skills():
                                        for field in required_fields:
                                            assert field in skill, f"Missing {field} in {agent.__class__.__name__}"
                        ...
                        with patch("agents.maps_agent.BaseA2AAgent.__init__", return_value=None):
                            with patch("agents.controller_agent.BaseA2AAgent.__init__", return_value=None):
                                ...
                                for agent in agents:
                                    ...
                                    assert len(ids) == len(set(ids)), f"Duplicate IDs in {agent.__class__.__name__}"
                            ...
                            with patch("agents.controller_agent.BaseA2AAgent.__init__", return_value=None):
                                ...
                                for agent in agents:
                                    for tool in agent._get_allowed_tools():
                                        assert tool.startswith("mcp__"), f"Tool {tool} doesn't follow naming convention"
                                        parts = tool.split("__")
                                        assert len(parts) == 3, f"Tool {tool} has wrong number of parts"
